...
% Tucker 做了数值层面的保形优化；TT 为 TT-SVD；Kron 采用模态指数的Strang分裂
%
% 输出：CV/CT 的位置RMSE曲线 + 运行时柱图 + 总结表
...
%% 真实轨迹 + 观测 =========
[true_cv, obs_cv] = gen_true_obs_cv(T, dt, sigma_process, sigma_obs);
[true_ct, obs_ct] = gen_true_obs_ct(T, dt, omega, sigma_process_ct, sigma_obs);

scenes = { ...
    struct('name','CV', 'true',true_cv, 'obs',obs_cv, 'dyn','cv', 'omega',0,     'sigma',sigma_process), ...
    struct('name','CT', 'true',true_ct, 'obs',obs_ct, 'dyn','ct', 'omega',omega, 'sigma',sigma_process_ct) ...
};
...
%% 绘图 =========
plot_compare_all(results, methods, scenes);

%% 文本表 =========
print_table(results, methods, scenes);
...
for t=1:T
    ...
    if t>1
        switch sc.dyn
            case 'cv', S = S.step_cv(S, D1,D2,D3,D4, dt, sc.sigma);
...
%% ============ 方法1：Tucker-FPE(HOSVD)（保形优化，不改流程） ============
...
%% ============ 方法2：TT-FPE（TT-SVD初始化 + 每步TT-SVD回投影） ============
...
%% ============ 方法3：Kron-Strang（密集PDF，克罗内克积分解近似） ============
...
S.step_ct = @(S,D1,D2,D3,D4,dt,om,sig) kron_step(S,D1,D2,D3,D4,dt,sig,om);
end
...
% 对称 Strang 分裂：E1(E2(E3(E4 P E4^T)E3^T)E2^T)E1^T（按模态“左乘”实现）
...
function [true_state, obs] = gen_true_obs_cv(T, dt, sigma_proc, sigma_obs)
...
%% ============ 绘图与表 ============
function plot_compare_all(results, methods, scenes)
% CT: 位置RMSE
figure(201); clf; hold on; grid on;
for m=1:numel(methods)
    tag = methods{m}.tag;
    plot(results.CT.(tag).pos_rmse,'LineWidth',1.8,'DisplayName',tag);
end
xlabel('Time step'); ylabel('Position RMSE'); title('CT: Position RMSE');
legend('Location','best');

% CV: 位置RMSE
figure(202); clf; hold on; grid on;
for m=1:numel(methods)
    tag = methods{m}.tag;
    plot(results.CV.(tag).pos_rmse,'LineWidth',1.8,'DisplayName',tag);
end
xlabel('Time step'); ylabel('Position RMSE'); title('CV: Position RMSE');
legend('Location','best');

% 运行时（总时长）
figure(203); clf;
cats = categorical({methods{1}.tag,methods{2}.tag,methods{3}.tag});
cats = reordercats(cats, cellstr(cats));
bar([ results.CV.(methods{1}.tag).time, results.CT.(methods{1}.tag).time; ...
      results.CV.(methods{2}.tag).time, results.CT.(methods{2}.tag).time; ...
      results.CV.(methods{3}.tag).time, results.CT.(methods{3}.tag).time ]);
set(gca,'XTickLabel',cellstr(cats)); grid on;
legend({'CV total(s)','CT total(s)'},'Location','best');
ylabel('Seconds'); title('Runtime Comparison');

% 每步平均
figure(204); clf;
bar([ results.CV.(methods{1}.tag).avg_time, results.CT.(methods{1}.tag).avg_time; ...
      results.CV.(methods{2}.tag).avg_time, results.CT.(methods{2}.tag).avg_time; ...
      results.CV.(methods{3}.tag).avg_time, results.CT.(methods{3}.tag).avg_time ]);
set(gca,'XTickLabel',cellstr(cats)); grid on;
legend({'CV avg(s/step)','CT avg(s/step)'},'Location','best');
ylabel('Seconds/step'); title('Average Runtime per Step');
end

function print_table(results, methods, scenes)
fprintf('\n===== Summary Table =====\n');
